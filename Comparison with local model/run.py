...
def merge_metrics(*results_dicts):
    models = ["gpt-4o", "Phi", "llamaLocal"]  # Lista dei modelli
...
excel_file="xlsx/input.xlsx" # Nome del file excel di input da cui prende domanda e ground truth
...
for request in data:
    ...
    try:
        ...
        product_result=product.find_products(product_context , 10)
        ...
        llamaLocal_result=writer_llamaLocal.write(product_context, product_result, assignment_context) # Risposta usando Phi-3-small
        ...
        metrics_phi=eval.evaluation_metrics(request, outfile, phi_result, "Phi") # Calcolo metriche di valutazione della risposta di Phi-3-Small
        metrics_llamaLocal=eval.evaluation_metrics(request, outfile, llamaLocal_result, "llamaLocal") # Calcolo metriche di valutazione della risposta di llamaLocal
        ...
        all_metrics = merge_metrics(metrics_gpt,metrics_phi,metrics_llamaLocal)
